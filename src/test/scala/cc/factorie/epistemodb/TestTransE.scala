...
//    }
    ...
    for (seed <- 0 until 2) {
      ...
      val trainer5 = new TransETrainer(regularizer, stepsize, dim, margin, mTrain, model5, random)
      trainer5.train(5)
      println("--")
      val model10 = TransEModel.randomModel(numCols, rowToEnts, numTopics, random)
      val trainer10 = new TransETrainer(regularizer, stepsize, dim, margin, mTrain, model10, random)
      trainer10.train(10)
      println("--")
      val model25 = TransEModel.randomModel(numCols, rowToEnts, numTopics, random)
      val trainer25 = new TransETrainer(regularizer, stepsize, dim, margin, mTrain, model25, random)
      trainer25.train(25)
