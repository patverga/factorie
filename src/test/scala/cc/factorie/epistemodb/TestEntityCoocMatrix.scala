...
/**
 * Created by pat on 6/29/15.
 */
class TestEntityCoocMatrix {

  @Test def testEntityCooc() {
    val numRows = 1000
    val numCols = 10000
    val nnz = 100000
    ...
    for (seed <- 0 until 2) {
      ...
      val m = EntityPairCoocMatrix.randomOneZeroMatrix(numRows, numCols, nnz, random, numTopics, noise1).prune(1,1)._1
      println("nnz: " + m.nnz())
