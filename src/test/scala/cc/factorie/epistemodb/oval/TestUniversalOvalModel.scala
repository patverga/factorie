...
class TestUniversalOvalModel extends JUnitSuite  with util.FastLogging {
...
//      val model = UniversalOval.randomModel(numRows, numCols, dim, random)
//      val trainer = new UniversalOvalTrainer(regularizer, stepsize, dim, margin, m, model, random)
...
//    }
    ...
    for (seed <- 0 until 2) {
      ...
      val model0 = UniversalOval.randomModel(numRows, numCols, dim, random)
      val model5 = UniversalOval.randomModel(numRows,  numCols, dim, random)
      val trainer5 = new UniversalOvalTrainer(regularizer, stepsize, dim, margin, mTrain, model5, random)
      trainer5.train(5)
      println("--")
      val model10 = UniversalOval.randomModel(numRows, numCols, dim, random)
      val trainer10 = new UniversalOvalTrainer(regularizer, stepsize, dim, margin, mTrain, model10, random)
      trainer10.train(10)
